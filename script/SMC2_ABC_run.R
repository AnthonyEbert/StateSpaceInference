...
for(tp in 1:TT){
  y <- hawkes_simulator(true_states[tp], true_theta, y$history, tp * 10, tp * 10 + 10, Ni = Inf)
}
...
full_list <- SMC2_ABC(prior_sample, dprior = dHawkes, loss, loss_args = inp, Ntheta = Ntheta, Nx = Nx, pacc = pacc, cl = cl, dt = 10, ESS_threshold = 0.25, TT = TT, trans = trans, invtrans = invtrans, trans_args = trans_args)
...
ggplot(theta_df) + aes(x = value, weights = weight, col = factor(time)) + geom_density() + facet_wrap(~parameter, scales = "free") + ggtitle(pacc)
